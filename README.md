...
3. Upon receiving the `Data!22` request, the node returns al six SolarBit values in [W/m^2]; otherwise it replies `NO_DATA` when buffers are empty.
4. `thingspeak_url_15sec()` and `thingspeak_url_5min_cal()` prepare REST payloads and trigger `upload_to_thingspeak()` to publish accumulated averages.
...
- ThingSpeak uploads use `AVG_COUNT_THINGSPEAK_THRESHOLD_CALIBRATION` samples per batch.
...
- Google Drive project archive: https://drive.google.com/drive/folders/1yivz0Nfw8G2uwzFmgZQO70k2WL2UrMPG?usp=sharing
